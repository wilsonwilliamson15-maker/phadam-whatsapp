...
export async function assignChat(
  ...
  try {
    const { patientId, agentName } = req.body as AssignChatBody;
...
export async function sendAgentReply(
  ...
  try {
    const { patientId, messageText, agentName } = req.body as AgentReplyBody;
    ...
    const [newMessage, updatedPatient] = await prisma.$transaction([
