...
app.get('/health', async (_req, res) => {
    try {
        ...
        console.error('[Health Check Error]', error instanceof Error
            ? error.message
            : error);
...
async function dispatchAppointmentReminders() {
    ...
    for (const appointment of appointments) {
        ...
        for (const reminderMinutes of reminderWindowMinutes) {
            ...
            const reminderLabel = reminderMinutes === 720
                ? '12 hours before'
                : reminderMinutes === 360
                    ? '6 hours before'
                    : '1 hour before';
...
async function startServer() {
    await (0, auth_1.ensureSuperAdmin)();
    server = app.listen(PORT, '0.0.0.0', () => {
        ...
        console.info(`🌐 Frontend origins configured: ${allowedOrigins.join(', ')}`);
        console.info('========================================');
    });
}
...
async function shutdown(signal) {
    ...
    server.close(async () => {
        try {
            ...
            console.error('[Shutdown] Failed to disconnect Prisma cleanly.', error instanceof Error
                ? error.message
                : error);
            process.exit(1);
        }
